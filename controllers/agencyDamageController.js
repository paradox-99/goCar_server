...
const getAgencyDamageList = async (req, res) => {
    ...
    if (search) {
        params.push(`%${search}%`);
        where.push(`(dr.damage_id ILIKE $${params.length} OR dr.booking_id ILIKE $${params.length} OR c.brand ILIKE $${params.length} OR c.model ILIKE $${params.length} OR cu.name ILIKE $${params.length})`);
    }
    ...
    if (car_id && car_id !== 'All') {
        params.push(car_id);
        where.push(`dr.car_id = $${params.length}`);
    }
...
const getAgencyDamageDetail = async (req, res) => {
    ...
    try {
        const reportQ = await pool.query(`
            SELECT
                dr.*,
                c.brand, c.model, c.car_type, c.build_year, c.images, c.status AS car_status, c.rating AS car_rating,
        ...
        const [historyRes, paymentsRes] = await Promise.all([
            pool.query(`
                ...
                FROM damage_reports WHERE car_id = $1 ORDER BY report_date DESC
            `, [r.car_id]),
            pool.query(`
...
const getRepeatOffenderVehicles = async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            SELECT
                c.car_id, c.brand, c.model, c.car_type, c.images[1] AS car_image,
                COUNT(dr.damage_id)::int AS total_reports,
                COUNT(*) FILTER (WHERE dr.severity = 'High')::int AS severe_count,
                COUNT(*) FILTER (WHERE dr.severity = 'Medium')::int AS moderate_count,
                COUNT(*) FILTER (WHERE dr.severity = 'Low')::int AS minor_count,
                COALESCE(SUM(dr.estimated_cost), 0) AS total_estimated_cost,
                COALESCE(SUM(CASE WHEN ${IS_PAID_SUBQ} THEN dr.estimated_cost END), 0) AS total_paid,
                COALESCE(SUM(CASE WHEN NOT ${IS_PAID_SUBQ} AND dr.estimated_cost > 0 THEN dr.estimated_cost END), 0) AS total_unpaid,
            ...
            HAVING COUNT(dr.damage_id) > 1
            ORDER BY COUNT(dr.damage_id) DESC
        `, [agencyId]);
    ...
    try {
        const [typesRes, vehiclesRes] = await Promise.all([
            pool.query(`
                ...
                JOIN cars c ON dr.car_id = c.car_id
                WHERE c.agency_id = $1 AND dr.damage_type IS NOT NULL
                ORDER BY dr.damage_type
            `, [agencyId]),
            pool.query(`
                SELECT c.car_id, c.brand, c.model, c.car_type
                FROM cars c WHERE c.agency_id = $1 ORDER BY c.brand, c.model
            `, [agencyId])
        ]);
