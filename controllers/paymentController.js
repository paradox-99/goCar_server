...
const makePayment = async (req, res) => {
     ...
     sslcz.init(data).then(apiResponse => {
          ...
          const result = pool.query(query, [booking_id, driver_cost, dropoff_date, pickup_date, remaining_cost, total, total_rent_hours, user_id, vehicle_id]);
...
const paymentSuccess = async (req, res) => {
     ...
     sslcz.transactionQueryByTransactionId(data).then(apiResponse => {
          if (apiResponse.no_of_trans_found === 1) {
               ...
               const result = pool.query(query, [payment_id, tran_id, apiResponse.element[0].amount, apiResponse.element[0].bank_gw]);
               if (result.rowCount === 0) {
                    return res.status(200).json({ message: 'Failed To Create Payment.', code: 0 });
               }
...
const getPaymentInfo = async (req, res) => {
     ...
     sslcz.transactionQueryByTransactionId(data).then(apiResponse => {
          if (apiResponse.no_of_trans_found === 1) {
               ...
               const result = pool.query(query, [booking_id, payment_id, apiResponse.element[0].tran_date]);
               if (result.rowCount === 0) {
                    return res.status(200).json({ message: 'Failed To Create Booking Payment.', code: 0 });
               }
